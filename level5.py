...
def pause_window(runs):
    ...
    level=font.render("Level 5",1,black)
...
background = pygame.image.load(os.path.join(r"level_5_images\Background.png"))
...
stickman_rect = pygame.Rect(50, 370, stickman_width, stickman_height)
pause_rect = pygame.Rect(15, 20, play_width, play_height)
side_rect=pygame.Rect(1050,370,150,170)

pause_rect_hover = pygame.Rect(13, 18, play_width + 10, play_height + 10)

def window(pressed,count,rects):
    win = pygame.display.set_mode((win_width, win_height))
    pygame.display.set_caption("Stickman-----LEVEL-5")

    win.blit(background,(0,0))

    if count<90:
        level=level_font.render("LEVEL 5",1,black)
        win.blit(level,(300,200))
    count+=1

    win.blit(winflag,(100,120))
    ...
    if key_pressed[pygame.K_RIGHT] and stickman_rect.x+stickman_width < 1200 and stickman_rect.x > 0:
        ...
        if stickman_rect.y>=370 and stickman_rect.x<=900 and key_pressed[pygame.K_SPACE]:
            ...
            stickman_rect.x+=stickman_vel
            win.blit(stickman_skate,(stickman_rect.x,stickman_rect.y))
            if stickman_rect.x>1050 and stickman_rect.y==270:
                pass
            else:
                stickman_rect.y += stickman_jump_height
        elif stickman_rect.x>900 and stickman_rect.y==170:
            stickman_rect.x+=stickman_vel
            stickman_rect.y=370
            win.blit(stickman_skate, (stickman_rect.x, stickman_rect.y))
        else:
            if stickman_rect.colliderect(side_rect):
    ...
    #   left move
        ...
        elif stickman_rect.x<1055 and stickman_rect.y==270 and key_pressed[pygame.K_SPACE]:
            win.blit(stickman_left_skate, (stickman_rect.x, stickman_rect.y))
            stickman_rect.x=910
            win.blit(stickman_left_skate, (stickman_rect.x, stickman_rect.y))
            stickman_rect.y=170
            win.blit(stickman_left_skate, (stickman_rect.x, stickman_rect.y))
        elif stickman_rect.x+stickman_width<1050 and stickman_rect.y==270:
            stickman_rect.y=370
            stickman_rect.x-=stickman_vel
            win.blit(stickman_left_skate, (stickman_rect.x, stickman_rect.y))
        else:
    ...
    #   right and left jump
        ...
        if pressed==0:
            if stickman_rect.x<900 and stickman_rect.y>=370:
                win.blit(stickman_stand, (stickman_rect.x, stickman_rect.y))
            else:
                stickman_rect.y -= stickman_jump_height
                win.blit(stickman_stand, (stickman_rect.x, stickman_rect.y))
    ...
    if stickman_rect.x <100 and stickman_rect.y<=170:
        f = open(r"level_star\level5_star.txt", "w")
        f.write("three")
        levels.main()
