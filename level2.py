...
def pause_window(runs):
    ...
    level=font.render("Level 2",1,black)
...
background=pygame.image.load(os.path.join(r"level_2_images\Background.png"))
...
stickman_rect=pygame.Rect(100,342,stickman_width,stickman_height)
...
def window(pressed,count):
    ...
    pygame.display.set_caption("Stickman-----LEVEL-2")

    win.blit(background,(0,0))

    if count<90:
        level=level_font.render("LEVEL 2",1,black)
    ...
    if key_pressed[pygame.K_RIGHT] and stickman_rect.x < 950:
        ...
        if space_pressed[pygame.K_SPACE]:
            ...
            if stickman_rect.x>415 and stickman_rect.x+stickman_width<530:
    ...
    #   left move
        ...
        if space_pressed[pygame.K_SPACE]:
            ...
            if stickman_rect.x>420 and stickman_rect.x<525:
                pygame.mixer.music.load(os.path.join(r"level_2_images\Falling.mp3"))
                pygame.mixer.music.play()
                stickman_rect.x -= 20
                stickman_rect.y += 100
    ...
    if stickman_rect.x >= 950 and stickman_rect.y<=342:
        f = open(r"level_star\level2_star.txt", "w")
        f.write("three")
        levels.main()
