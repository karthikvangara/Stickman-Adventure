...
def pause_window(runs):
    ...
    level=font.render("Level 3",1,black)
...
background=pygame.image.load(os.path.join(r"level_3_images\Background.png"))
...
brick=pygame.transform.scale(pygame.image.load(os.path.join(r"level_3_images\Brick.png")),(brick_width,brick_height))
...
stickman_rect=pygame.Rect(100,330,stickman_width,stickman_height)
...
def window(pressed,count):
    ...
    pygame.display.set_caption("Stickman-----LEVEL-3")

    win.blit(background,(0,0))

    if count<90:
        level=level_font.render("LEVEL 3",1,black)
        win.blit(level,(300,200))
    count+=1

    win.blit(winflag,(950,285))

    win.blit(brick,(450,325))
    ...
    if key_pressed[pygame.K_RIGHT] and stickman_rect.x < 950:
        ...
        if key_pressed[pygame.K_SPACE] and (stickman_rect.x!=655 and stickman_rect.x!=660):
            stickman_rect.y-=stickman_jump_height
            stickman_rect.x+=stickman_vel
            win.blit(stickman_skate,(stickman_rect.x,stickman_rect.y))
            if stickman_rect.x!=400:
                stickman_rect.y += stickman_jump_height

        elif stickman_rect.x == 660 and stickman_rect.y==230:
            stickman_rect.y += stickman_jump_height

        elif stickman_rect.x>400 and stickman_rect.x+stickman_width<710 and stickman_rect.y>=330:
            pygame.mixer.music.load(os.path.join(r"level_3_images\Falling.mp3"))
    ...
    #   left move
        ...
        if key_pressed[pygame.K_SPACE] and (stickman_rect.x!=405 and stickman_rect.x!=408):
            ...
            if stickman_rect.x!=680:
                stickman_rect.y += stickman_jump_height

        elif stickman_rect.x == 408 and stickman_rect.y == 230:
            stickman_rect.x-=10
            stickman_rect.y += stickman_jump_height

        elif stickman_rect.x > 400 and stickman_rect.x < 670 and stickman_rect.y >= 330:
            pygame.mixer.music.load(os.path.join(r"level_3_images\Falling.mp3"))
            pygame.mixer.music.play()
            stickman_rect.x -= 20
            stickman_rect.y += 100
    ...
    if stickman_rect.x >= 950 and stickman_rect.y<=342:
        f = open(r"level_star\level3_star.txt", "w")
        f.write("three")
        levels.main()
